...
from src.core_context.auth.infrastructure.mapper import (
    apply_domain_to_user_orm,
    refresh_token_orm_to_domain,
    user_orm_to_domain,
)
from src.core_context.user.infrastructure.orm import RefreshToken, User
...
class SqlAlchemyRefreshTokenRepository(IRefreshTokenRepository):
    ...
    async def save(self, token: RefreshTokenDomain) -> None:
        await self._db.execute(update(RefreshToken).where(RefreshToken.id == token.id).values(is_revoked=token.is_revoked))
        await self._db.commit()
