from src.core_context.auth.application.commands import ResendVerificationCommand, VerifyEmailCommand
from src.core_context.auth.domain.errors import UserNotFoundError
from src.core_context.auth.domain.events import VerificationEmailRequestedEvent
from src.core_context.auth.domain.interfaces import IMailService, IUserRepository
...
class ResendVerificationCommandHandler:
    ...
    async def handle(self, cmd: ResendVerificationCommand) -> None:
        ...
        if user and not user.is_verified:
            await self._event_bus.publish(VerificationEmailRequestedEvent(user_id=user.id, email=user.email, name=user.name))
