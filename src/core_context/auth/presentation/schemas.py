...
class RefreshRequest(BaseModel):
    ...
    def to_refresh_command(self) -> RefreshTokenCommand:
        from src.core_context.auth.application.commands import RefreshTokenCommand

        return RefreshTokenCommand(refresh_token=self.refresh_token)

    def to_logout_command(self) -> LogoutCommand:
        from src.core_context.auth.application.commands import LogoutCommand
...
class ChangePasswordRequest(BaseModel):
    ...
    def to_command(self, user_id: int) -> ChangePasswordCommand:
        from src.core_context.auth.application.commands import ChangePasswordCommand
    ...
    def to_reset_command(self, token: str) -> ResetPasswordCommand:
        from src.core_context.auth.application.commands import ResetPasswordCommand
...
class Verify2FARequest(BaseModel):
    ...
    def to_command(self, device_info: str) -> Verify2FAAndLoginCommand:
        from src.core_context.auth.application.commands import Verify2FAAndLoginCommand
