...
from src.core_context.auth.application.handlers.logout import LogoutCommandHandler
...
from src.core_context.auth.application.handlers.reset_password import ForgotPasswordCommandHandler, ResetPasswordCommandHandler
...
from src.core_context.auth.application.handlers.verify_email import ResendVerificationCommandHandler, VerifyEmailCommandHandler
...
async def get_reset_password_handler(
    user_repo=Depends(get_user_repo), token_repo=Depends(get_token_repo), mail_service=Depends(get_mail_service)
...
async def get_verify_2fa_handler(
    user_repo=Depends(get_user_repo), token_repo=Depends(get_token_repo), totp_service=Depends(get_totp_service)
) -> Verify2FAAndLoginCommandHandler:
