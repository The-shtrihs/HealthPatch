...
async def get_register_uc(
    ...
    return RegisterUseCase(user_repo, MailService(), _pw)
...
async def get_verify_email_uc(user_repo=Depends(get_user_repo)) -> VerifyEmailUseCase:
    return VerifyEmailUseCase(user_repo, MailService())

async def get_resend_verification_uc(user_repo=Depends(get_user_repo)) -> ResendVerificationUseCase:
    return ResendVerificationUseCase(user_repo, MailService())

async def get_forgot_password_uc(user_repo=Depends(get_user_repo)) -> ForgotPasswordUseCase:
    return ForgotPasswordUseCase(user_repo, MailService())

async def get_reset_password_uc(
    user_repo=Depends(get_user_repo),
    token_repo=Depends(get_token_repo),
) -> ResetPasswordUseCase:
    return ResetPasswordUseCase(user_repo, token_repo, MailService(), _pw)
...
async def get_enable_2fa_uc(user_repo=Depends(get_user_repo)) -> Enable2FAUseCase:
    return Enable2FAUseCase(user_repo, TotpService())

async def get_confirm_2fa_uc(user_repo=Depends(get_user_repo)) -> Confirm2FAUseCase:
    return Confirm2FAUseCase(user_repo, TotpService())

async def get_disable_2fa_uc(user_repo=Depends(get_user_repo)) -> Disable2FAUseCase:
    return Disable2FAUseCase(user_repo, TotpService())

async def get_verify_2fa_uc(
    user_repo=Depends(get_user_repo),
    token_repo=Depends(get_token_repo),
) -> Verify2FAAndLoginUseCase:
    return Verify2FAAndLoginUseCase(user_repo, token_repo, TotpService())
